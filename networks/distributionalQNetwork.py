...
class DistributionalQNetwork(nn.Module):
    def __init__(self, n_actions, input_dims, file_name, lr=0.0001, n_atoms=51, checkpoint_dir="models/Unknown"):
        super().__init__()
        ...
        self.fc2 = nn.Linear(512, n_actions * n_atoms) # the output here is the return distrubtion, that's
        # support is made up of n_atoms atoms and ranges from v_min -> v_maxvalues of each action, given an input state (game frame)
    ...
    def forward(self, state):
        ...
        logits = self.fc2(fc1).view(-1, self.n_actions, self.n_atoms) # reshape to (batch_size, n_actions, n_atoms)
