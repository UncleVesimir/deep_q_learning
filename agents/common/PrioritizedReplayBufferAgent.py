...
class ReplayBufferAgent(BaseAgent):
    def __init__(self, mem_size=100_000,*args, **kwargs):
        ...
        self.memory = PERBuffer(self.mem_size, self.input_dims, self.n_actions)
    ...
    def sample_from_buffer(self):
        states, actions, rewards, next_states, terminals, _, w = self.memory.sample_from_buffer(self.batch_size)
        ...
        w = torch.tensor(w).to(self.q_eval.device)

        return states, actions, rewards, next_states, terminals, w